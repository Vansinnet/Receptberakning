...
function updateMedListStatuses() {
  ...
  list.querySelectorAll('.med-item').forEach(btn => {
    ...
    const hasInteraction = s.interactionWarnings && s.interactionWarnings.length > 0;
    ...
    if (dot) {
      dot.className = 'status-dot' + (hasInteraction ? ' interaction' : (s.valid ? (isWarnDot ? ' warn' : ' ok') : ''));
      const sr = dot.querySelector('.sr-only');
      if (sr) sr.textContent = hasInteraction ? 'Interaktion identifierad' : (s.valid ? (isWarnDot ? '\u00c5tg\u00e4rd kr\u00e4vs' : 'OK att f\u00f6rnya') : '');
    }
...
function buildMedList() {
  ...
  for (let i = 0; i < states.length; i++) {
    ...
    const dot    = el('span', { cls: 'status-dot' + (hasInteraction ? ' interaction' : (s.valid ? (isWarnDot ? ' warn' : ' ok') : '')) });
    if (s.valid || hasInteraction) dot.appendChild(el('span', { cls: 'sr-only', text: hasInteraction ? 'Interaktion identifierad' : (isWarnDot ? '\u00c5tg\u00e4rd kr\u00e4vs' : 'OK att f\u00f6rnya') }));
...
function checkAllInteractions() {
  ...
  for (var i = 0; i < states.length; i++) {
    if (states[i] && states[i].atcCode) {
      atcEntries.push({ i: i, a: states[i].atcCode });
    }
  }

  var warnings = CHECK_INTERACTIONS(atcEntries);

  for (var j = 0; j < states.length; j++) {
    var drugWarnings = [];
    for (var w = 0; w < warnings.length; w++) {
      if (warnings[w].drugs[0] === j || warnings[w].drugs[1] === j) {
        drugWarnings.push(warnings[w]);
      }
    }
    applyMedStatePatch(j, { interactionWarnings: drugWarnings });
  }
...
function renderInteractionAlerts() {
  var container = _el('interactionAlerts');
  if (!container) return;
  ...
  for (var i = 0; i < states.length; i++) {
    var s = states[i];
    if (!s || !s.interactionWarnings) continue;
    for (var w = 0; w < s.interactionWarnings.length; w++) {
      var warn = s.interactionWarnings[w];
      var key = warn.t + '|' + warn.drugs[0] + '|' + warn.drugs[1];
      if (!seen[key]) {
        seen[key] = true;
        uniqueWarnings.push(warn);
      }
    }
  }
  ...
  for (var w = 0; w < uniqueWarnings.length; w++) {
    var warn = uniqueWarnings[w];
    var names = warn.drugs.map(function(i) {
      return states[i] ? (states[i].medName || 'L\u00e4kemedel ' + (i + 1)) : '';
    }).filter(Boolean).join(' + ');

    var alertEl = el('div', { cls: 'interaction-alert interaction-' + (warn.s === 'danger' ? 'danger' : 'warn') });

    var header = el('div', { cls: 'interaction-header' });
    header.appendChild(el('span', { cls: 'interaction-icon', text: '\u26A0' }));
    header.appendChild(el('strong', { text: warn.t }));
    header.appendChild(el('span', { cls: 'interaction-drugs', text: names }));
    alertEl.appendChild(header);

    alertEl.appendChild(el('p', { cls: 'interaction-desc', text: warn.d }));
    alertEl.appendChild(el('p', { cls: 'interaction-rec', text: warn.r }));
...
function renderResultForMed(i) {
  ...
  /* Interaktionsvarningar — globala, visas oavsett aktivt kort */
  renderInteractionAlerts();
...
function renderAutocomplete(results) {
  ...
  for (let r = 0; r < results.length; r++) {
    ...
    const unitDisplay = drug.c
      ? drug.p + ' ' + (drug.u || 'st') + ' · ' + drug.f + ' (manuell bed\u00f6mning)'
      : drug.p + ' ' + (drug.u || 'st') + ' \u00b7 ' + drug.f;
