...
// Skapar debounce-instans utan att anropa — används vid ombyggnad efter borttagning
// av kort (clearCurrentCard) där calc ska vänta på nästa användarinput.
...
// Sammanslagen ensureDebounce + anrop — ersätter det tidigare mönstret
// ensureDebounce(i); calcDebounced[i]() för den vanliga input-hanteraren.
function debouncedCalc(i) {
  ensureDebounce(i);
  calcDebounced[i]();
}
...
function clearCurrentCard() {
  ...
  if (states.length > 1) {
    ...
    // calcDebounced-closures innehåller fasta idx-värden — avbryt väntande timers och bygg om
...
/* Kopiering — använder gemensam copyToClipboard från utils_test.js */
function copyResultText() {
  const body = getEl('copyBodyResult');
  copyToClipboard(body ? body.textContent : '', getEl('copyBtnResult'));
}
...
// Rensa vid pagehide (bfcache-säkerhet) — använder _resetLongtermDOM från longterm_test.js
// för att undvika duplicerad rensningslogik mellan pagehide och clearLongterm.
