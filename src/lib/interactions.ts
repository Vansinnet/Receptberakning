...
// ── ATC prefix-index (lat byggt vid första CHECK_INTERACTIONS-anrop) ──
let _idxA: Map<string, number[]> | null = null;
let _idxB: Map<string, number[]> | null = null;
...
function _ensureIndex() {
  ...
  for (let i = 0; i < INTERACTIONS.length; i++) {
    for (const p of INTERACTIONS[i].a) {
      let arr = _idxA.get(p); if (!arr) { arr = []; _idxA.set(p, arr); } arr.push(i);
    }
    for (const p of INTERACTIONS[i].b) {
      let arr = _idxB.get(p); if (!arr) { arr = []; _idxB.set(p, arr); } arr.push(i);
    }
  }
}
  ...
  for (let x = 0; x < atcEntries.length; x++) {
    for (let y = x + 1; y < atcEntries.length; y++) {
      ...
      for (const ruleIdx of candidateIndices) {
        ...
        const key = `${ruleIdx}|${ix.t}|${atcEntries[x].i}|${atcEntries[y].i}`;
        if (seen.has(key)) continue;
        seen.add(key);
        warnings.push({
          s: ix.s, t: ix.t, d: ix.d, r: ix.r,
